...
def compute_random_walk_loss(
        ...
        point_img_labels: List[int]=None, step_weights: Tensor=None, text_feat=None, att=None, att_creative=None) -> Tensor:
    """
    ...
    text_feat = Variable(text_feat).cuda()
    # print(text_feat.shape)
    # proto_images = proto_images.cuda()
    # import pdb; pdb.set_trace();
    if config.space == 'feature':
        points_for_protos = discriminator(proto_imgs.cuda(), att)
        points = discriminator(point_imgs.cuda(), att_creative)
...
def compute_transition_probs(points: Tensor, protos: Tensor, similarity_measure: str, dot_product_scale: float=3.0) -> Tensor:
    ...
    # import  pdb; pdb.set_trace();
    points_to_points_sims -= Variable(torch.eye(n_points).cuda().float() * 1000)
    ...
    point_to_proto_probs = F.softmax(points_to_protos_sims, dim=1) # [n_points, n_protos]
    proto_to_point_probs = F.softmax(points_to_protos_sims.t(), dim=1) # [n_protos, n_points]
    point_to_point_probs = F.softmax(points_to_points_sims, dim=1) # [n_points, n_points]
...
def compute_walker_loss(
    ...
    for step, curr_alpha in zip(range(config.num_steps + 1), step_weights):
        ...
        if not isinstance(curr_p2p_probs, Variable):
            curr_p2p_probs = Variable(curr_p2p_probs)
        curr_transition = torch.mm(torch.mm(proto_to_point_probs, curr_p2p_probs) , point_to_proto_probs)
        curr_transition = torch.clamp(curr_transition, 1e-10, 1 - 1e-10)

        probs = curr_transition.diag() if target_dist == 'diag' else curr_transition.view(-1)
        ...
        curr_p2p_probs = torch.mm(curr_p2p_probs, point_to_point_probs)
...
def compute_rw_creative_loss(config, dataset, netD, G_creative_sample, netG, att_creative) -> Tensor:
    """
    Computes creative random walk loss, i.e. tries to classify badly creative Generator's samples
    """
    # blobs = data_layer.forward()
    # Manually genearting labels of the kind [1,1,1,2,2,2,3,3,3,...] to compute prototypes
    labels = torch.arange(dataset.ntrain_class).unsqueeze(1).repeat(1, config.n_examples_per_proto).view(-1)
    # import pdb; pdb.set_trace();
    text_feat = np.array([dataset.train_feature[int(i), :].numpy() for i in labels])
    text_feat = torch.from_numpy(text_feat.astype('float32')).cuda()
    z = Variable(torch.randn(len(labels), config.nz).cuda())
    # import pdb; pdb.set_trace();
    att = Variable(torch.Tensor(np.array([dataset.attribute[int(i)].numpy() for i in labels]))).cuda()
    G_sample = netG(z, c=att)  # text feats.
    # step_weights = netG.get_alpha() if config.rw_params.learnable_alpha else generate_step_weights(config.rw_params)
    ...
    rw_creative_walker_loss, rw_creative_visit_loss = compute_random_walk_loss(
        config, netD, G_sample, labels, G_creative_sample, 'uniform', text_feat=text_feat, att=att, att_creative=att_creative)

    assert np.isfinite(rw_creative_walker_loss.detach().data.cpu().numpy()), f"bad rw_creative_walker_loss: {rw_creative_walker_loss}"
    assert np.isfinite(rw_creative_visit_loss.detach().data.cpu().numpy()), f"bad rw_creative_visit_loss: {rw_creative_visit_loss}"
...
def generate_step_weights(config):
    ...
    return torch.Tensor([0.7 ** n for n in range(config.num_steps + 1)])